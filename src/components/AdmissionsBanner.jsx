...
export default function AdmissionsBanner() {
  ...
  // Hidden while settings are loading, when admissions are closed
  // (settings.is_open === false, admin-controlled), or on the
  // admissions page itself.
  const hidden = loading || !settings || !settings.is_open || pathname === '/admissions';
  ...
  return (
    ...
    >
      <style>{`
        ...
        .bell-ring { transform-origin: top center; animation: bell-ring 4s ease-in-out infinite; }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-2.5 flex items-center justify-center gap-3 sm:gap-4 flex-wrap text-center">
        ...
        <span className="relative hidden sm:flex w-10 h-10 shrink-0 items-center justify-center bg-[#D27D2D]">
          <Bell size={26} strokeWidth={3} className="bell-ring text-[#FF0000]" />
          <span className="absolute -top-1.5 -right-1.5 w-3.5 h-3.5 rounded-full bg-[#FF0000] dot-pulse" />
        </span>

        {/* Admissions Status — year comes from admissions_settings, so it
            changes automatically the moment an admin updates the intake year */}
        <span className="font-display text-[11px] sm:text-xs tracking-[0.15em] uppercase text-[#F4F4F4]">
          {settings.intake_year} Admissions Open
        </span>

        <span className="hidden sm:inline text-[#F4F4F4]/30">·</span>

        <span className="text-[11px] sm:text-xs text-[#F4F4F4]/60">
          Grade 8–12 · Limited space
        </span>
